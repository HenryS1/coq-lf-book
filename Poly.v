...
Theorem app_nil_r : forall (X:Type), forall l:list X,
  l ++ [] = l.
Proof. intros X l. induction l as [|e l' IHl'].
       - reflexivity.
...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intros A l m n. induction l as [|e l' IHl'].
  - reflexivity.
  - simpl. rewrite <- IHl'. reflexivity.
Qed.
...
Proof.
  intros X l1 l2. induction l1 as [|e l1' IHl1'].
  - reflexivity.
...
Proof. intros X y f l1 l2. induction l1 as [|e l1' IHl1'].
       - reflexivity.
       - simpl. rewrite IHl1'. reflexivity.
Qed.
...
Proof.
  intros X Y f l. induction l as [|e l' IHl'].
  - reflexivity.
  - simpl. rewrite <- IHl'. replace ([f e]) with (map f [e]). rewrite map_distr.
...
Fixpoint flat_map {X Y: Type} (f: X -> list Y) (l: list X)
  ...
  match l with 
  | nil => nil
  | e :: rest => app (f e) (flat_map f rest)
  end.
...
Definition fold_map {X Y: Type} (f: X -> Y) (l: list X) : list Y :=
  fold (fun e new_l => f e :: new_l) l nil.
...
By induction on l
if l = [] then nth_error l = None
if l = a :: l' and n == length l then n > 0 
so nth_error l n = nth_error l' (pred n) where (pred n) is the length of l'
the induction hypothesis states that for all lists l' smaller that l
if m = length l' then nth_error l' m = None. Because (pred n) = length l' 
nth_error l' (pred n) = None.
Qed
